...
#print open('model.json').read().strip()
#model = model_from_json(open('model.json').read().strip())
#model.load_weights("model_weights.h5", by_name=True)

#model.trainable=False
#model.get_layer('bn_conv1').trainable = False
...
intermediate_model = Model(inputs=model.layers[0].input,
        outputs=[l.output for l in model.layers[2:4]])
...
print intermediate_model.get_weights()
print "layer weights"
#print model.get_layer("conv1").get_weights()
print "test_generator"
print test_generator[0]
print model.predict(test_generator[0], batch_size=len(test_generator[0]))
